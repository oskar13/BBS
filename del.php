	...
	if ($_SESSION['admin_level'] >= DEL_LEVEL) {
		if (isset($_REQUEST['post_ID'])) {
			...
			if (isset($_REQUEST['del_post'])) {
			    ...
			    if (!$result['parent_ID']) {
			    	try {
				        ...
				        WHERE parent_ID = :post_ID AND pic_ID IS NOT NULL");
				        $stmt->execute(array('post_ID' => $_REQUEST['post_ID']));
			...
			if (isset($_REQUEST['sticky'])) {
				if (!isset($_REQUEST['sticky_level'])) {
					$_REQUEST['sticky_level'] = 1;
				}
				try {
				    ...
				    $stmt->execute(array('post_ID' => $_REQUEST['post_ID'], 'sticky_level' => $_REQUEST['sticky_level']));
